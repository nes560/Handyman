...
// Ambil Data Order dari Server (Menggunakan dummy data untuk preview)
async function loadOrders(status) {
    ...
    let orders = getDummyOrders(status); 
    ...
    if (orders.length === 0) {
        list.innerHTML = `
            <div class="col-span-full flex flex-col items-center justify-center py-10 text-slate-400 bg-white rounded-2xl border border-slate-100 border-dashed">
                ...
                <p class="text-sm font-medium">Belum ada orderan "${status}".</p>
            </div>`;
        return;
    }
...
function getActionButtons(status, orderId) {
    if (status === 'pending') {
        return `
            ...
            <button class="flex-1 border border-red-100 bg-red-50 text-red-500 py-2 rounded-lg text-xs font-bold hover:bg-red-100 transition">Tolak</button>
        `;
...
function createOrderCard(order, buttons) {
    const formattedDate = new Date(order.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' });
    return `
        <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition mb-3">
            <div class="flex justify-between mb-2">
                <span class="bg-blue-50 text-blue-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase">${order.kategori_jasa}</span>
                <span class="text-[10px] text-slate-400">${formattedDate}</span>
            </div>
            <h3 class="font-bold text-slate-800 text-sm">${order.nama_user}</h3>
            <p class="text-xs text-slate-500 mb-3 line-clamp-2">"${order.deskripsi_masalah}"</p>
            <p class="text-xs text-slate-600 mb-3 flex items-center gap-1"><i class="fa-solid fa-location-dot text-red-400"></i> ${order.alamat}</p>
            <div class="flex gap-2 pt-2 border-t border-slate-50">${buttons}</div>
        </div>
    `;
}

// Update Status Order (Simulasi)
async function updateStatus(id, newStatus) {
    const msg = newStatus === 'proses' ? 'Terima pekerjaan ini?' : 'Selesaikan pekerjaan ini?';
    if(!confirm(msg)) return;

    // Simulasi Sukses 
    showToast(newStatus === 'proses' ? "Order Diterima!" : "Order Selesai!", "SUCCESS");
    
    // Refresh Tab (Pindah tab jika Terima)
    if(newStatus === 'proses') switchTab('proses');
    else loadOrders(currentTab);
}
...
function logout() {
    if(confirm('Apakah Anda ingin mengakhiri sesi kerja?')) {
        ...
        // Arahkan ke halaman login (diasumsikan login.html)
